...
**Pending next action:** Stage 02 is complete and verified; continue with Stage 03, which checks the standardized visit labels against the recorded relative day `LBDT_PC` under each trial's own time origin. A10 modeling remains separate. Continue the PDS-first user-side source search; no dbGaP application. Affiliation and Downloads duplicate housekeeping remain unanswered/unverified.
...
| A9 | Prepare the audited data in explicitly authorized stages | PARTIAL: Stage 01 completed 2026-09-06 (roster and joins). Stage 02 completed 2026-09-06 (visit vocabularies standardized, all 210,442 rows dispositioned, zero dropped). Suite is 43 tests, all passing. Stages 03 to 07 authorized and outstanding. Integrity review remains required before features |
