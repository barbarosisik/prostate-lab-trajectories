...
**Pending next action:** all seven authorized preparation stages are complete and verified. The pipeline has reached the integrity checkpoint and stops here for review before any feature is calculated. Two matters must be settled at that review: the laboratory and outcome clocks use different reference days, and LDH plus sodium have materially thinner coverage than the rest of the panel. A10 modeling remains separate. Continue the PDS-first user-side source search; no dbGaP application. Affiliation and Downloads duplicate housekeeping remain unanswered/unverified.
