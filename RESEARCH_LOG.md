...
| ID | Date | Decision | Reason | Status |
|---|---|---|---|---|
| D-035 | 2026-09-06 | Continue preparation stages 02 to 07 sequentially toward cleaned data after publishing the current permitted work. Retain raw fields, explicit uncertainty, per-stage tests and the integrity checkpoint before features. | The user explicitly requested continuing cleaning one step at a time and function-level documentation. | Verified complete 2026-09-06; stages 02 to 07 executed and the integrity checkpoint passed |

### Differential reconciliation, 2026-09-07

| ID | Date | Decision | Reason | Status |
|---|---|---|---|---|
| D-036 | 2026-09-07 | Keep all ten white cell differential codes. Treat the five `...LE` codes as shares of the white cell count rather than as duplicate counts, name them distinctly, and record each one in `DERIVED_FROM` as a function of its own count and `WBC` so that no feature or model may use a share as evidence independent of the two numbers it is computed from. Drop nothing. | The catalogue, the values and the family arithmetic all say these are two different quantities, so dropping either would discard real information. But the share is arithmetically determined by the count and the white cell count, so using all three as separate inputs would let one measurement corroborate itself. Recording the dependency keeps both without that risk. | Verified complete; 23 new tests, suite now 203 and all passing |

### Proposed feature plan, 2026-09-07
...
### 2026-09-07: The five duplicated differential codes reconciled, Q-021 closed

- **Verified question settled.** NEULE, LYMLE, MONOLE, EOSLE and BASOLE are not duplicates of NEU, LYM, MONO, EOS and BASO. Each pair is one cell type recorded two ways: an absolute count of cells in a volume of blood, standardized to `10^9/L`, and that cell type's share of all white cells, standardized to `%`. The catalogue names them accordingly, for example `NEUTROPHILS` against `NEUTROPHILS/LEUKOCYTES`, and both sit in the same `HEMATOLOGY` category and the same `DIFFERENTIAL` subcategory.
- **Verified by value, not by name alone.** Across 14,380 readings where a count and its share were drawn from the same patient, visit and collection day, every single one reproduces `100 * count / WBC` to within 1.0 percentage point. The largest disagreement anywhere in the family is 0.45 percentage points, so the agreement is not marginal. The reverse hypothesis, that the share is the count, fails by two to three orders of magnitude.
- **Verified by the family arithmetic.** Where all five counts and a white cell count are present, 2,810 of 2,816 readings total the white cell count within one per cent. Where all five shares are present, 2,858 of 2,891 sum to a hundred within one point. A differential is a partition of the white cells, and it behaves like one here.
- **Verified consequence, which is the reason the question mattered.** Both forms are kept because a count and a share answer different questions: a patient whose white cells all halve keeps every percentage unchanged while every count falls. But the share carries no information beyond its count and the white cell count, so `DERIVED_FROM` now records that dependency and any later feature work must treat the three as two quantities.
- **Verified scope of the codes.** All five share codes come from CELGENE only, cover 519 patients and 2,891 usable readings each, and are a subset of what the counts already cover. NEU is measured in all three trials with 5,887 usable readings across 1,549 patients, so the shares widen no cohort.
- **Verified secondary finding, recorded and not acted on.** 75 readings carry a usable NEULE where the matching NEU was left `conflict_unresolved` by Stage 06, meaning two different neutrophil counts were recorded on the same day with no basis to choose. The share plus the white cell count could settle those, and one patient's entire usable NEU record depends on it. This is a possible later decision, not a change made now, because resolving a contested value is a data decision that needs its own approval.
- **Verified non-finding on an earlier worry.** 519 NEU rows in EFC6546 were originally reported as percentages, but the sponsor converted every one of them into `10^9/L` before standardizing; none kept a raw percentage. The standardized column is not contaminated, which supports the existing finding that units were already harmonized upstream.
- **Verified tests:** 23 new invented-data tests. They check the correct verdict, and also that a genuinely duplicated pair is reported as duplicated rather than explained away, that a relationship fitting neither description halts the run, that a share is never compared against a count drawn on a different day, and that a share of zero white cells is undefined rather than zero. Suite is 203, all passing.
- **Added D-036.** Highest existing decision identifier is now D-036; the earlier notes claiming D-034 were stale, since D-035 was already present.
- **Next:** present the feature plan for approval. No feature has been built.

### 2026-09-07: Feature plan proposed for approval, with the cohort design changed on evidence

- **Verified reason for changing the cohort design.** The certified 574-patient cohort requires all 18 primary tests at cycles 2, 3 and 4 at once. Measured consequence: it retains 6 of the 197 recorded treatment discontinuations, because a patient who stops treatment stops producing pre-cycle blood tests. Requiring one cycle at a time retains 39 at cycle 2, 25 at cycle 3 and 7 at cycle 4. The gradient is the selection made visible.
- **Verified landmark cohort sizes and event counts,** each requiring all 18 primary tests at that one cycle plus a full baseline: cycle 1, 439 patients and 120 deaths; cycle 2, 786 and 339; cycle 3, 782 and 333; cycle 4, 735 and 304. Every one is larger than the 574-patient intersection, which holds 242 deaths.
- **Verified that the landmark cohort is not a healthier selection on survival.** Death rate 43.1 per cent against the roster's 41.4 per cent, median last-known day 392 against 387. Survival work at the cycle-2 landmark is therefore defensible. Tolerance work is not, and is proposed as exploratory only.
- **Verified who is excluded and why.** Of 814 roster patients outside the cycle-2 landmark cohort, all 476 ASCENT2 patients are excluded because that trial records only PSA at cycle visits, plus 216 EFC6546 and 122 CELGENE for incomplete panels. A separate 168 patients hold no usable cycle-2 measurement at all, and those are the sickest group: 87 of them discontinued and 32 were last known alive within 84 days.
- **Verified that ASCENT2 contributes zero patients to any multi-test cohort,** at any cycle, which resolves Q-012. It contributes 367 patients at the cycle-2 landmark to a PSA-only analysis, and is the only route to a genuine third trial for validation.
- **Verified endpoint strength in the cycle-2 landmark cohort.** Overall survival: 339 deaths, usable. `DISCONT` equal to 1: 39 events with 96 missing, too few for anything but exploratory reporting, which bears on Q-017. `ENDTRS_C` reason for stopping: 307 progression, 122 adverse event, 260 possible adverse event, 96 miscellaneous, 1 completed, usable as a categorical target but not as a dated event.
- **Verified trial imbalance that forces stratification.** In the intersection cohort, CELGENE deaths are 11.1 per cent with median last-known day 280, against EFC6546 at 70.0 per cent and 730. Pooling without a trial stratum would let a model learn the trial rather than the patient. The landmark cohorts are at least balanced in size, 404 CELGENE against 382 EFC6546 at cycle 2.
- **Proposed feature set, deliberately small.** Per test, at landmark cycle k: the value at cycle k, the change from baseline to cycle k, and where k is 3 or more a single fitted slope across baseline to cycle k. That is 36 candidate features at cycle 2 and 54 at each of cycles 3 and 4, 144 in total, against the several hundred that an unconstrained set would produce. Cycle-1 values are not used as a previous-cycle reference because EFC6546 cycle 1 is nearly empty, so at cycle 2 the change from baseline is the only trend available.
- **Proposed control of candidate signals, addressing Q-022.** One primary endpoint and one primary landmark, giving 36 primary candidates; Benjamini-Hochberg false discovery rate control at 5 per cent within each endpoint and landmark family; every other combination labelled exploratory; a primary finding required to hold in CELGENE and EFC6546 separately, not only pooled; and all results reported including null and contradictory ones.
